...
//  	footerFn();//footer
	...
	if("1" == cateDepthInfo){
		loadTemplate("reservation",'dvd');	// 예약상품 상품 조회	
	}else{
		...
		$.ajax({
	        ...
	        ,success : function(data){
                ...
                if(!isEmpty(data)) {
	                ...
	                var obj = document.getElementById("dvdPaging");
	                var hndl = function(page){ return function(e){ 
                			e.preventDefault();
                			$('#dvd_curPage').val($(this).attr("href").replace("#",""));
                			goList($('#dvd_curPage').val());
                		};
                	};
...
<body>
	<input type="hidden" id="dvd_curPage" value="1">
	<!-- content -->
	<div id="content">
		<div class="category_wrap">

			<!-- DVD / 블루레이 탭 신규 추가 -->
            <div class="tab font_t">
                <ul>
                    <li <c:if test="${fn:substring(ctgrId,0,6) eq '000401'}">class="active"</c:if>><a href="javascript:;" onclick="location.href='/m/dvd/biz/category/000401';return false;">DVD</a></li>
                    <li <c:if test="${fn:substring(ctgrId,0,6) eq '000400'}">class="active"</c:if>><a href="javascript:;" onclick="location.href='/m/dvd/biz/category/000400';return false;">BLU-RAY</a></li>
                </ul>
            </div>
            <!--/ DVD / 블루레이 탭 신규 추가 -->
				...
				<c:choose>
					<c:when test="${cateDepthInfo eq '1'}">
						<div class="tab type02" id="searchListGbn">
							<ul>
								<li id="reservation" class="MB_SIDE_CATE_15_PRE active"><a href="javascript:;" onclick="loadTemplate('reservation','dvd'); return false;">예약상품</a></li>
								<li id="new" class="MB_SIDE_CATE_15_NEW"><a href="javascript:;" onclick="loadTemplate('new','dvd'); return false;">새로 나온 상품</a></li>
								<li id="best" class="MB_SIDE_CATE_15_BEST"><a href="javascript:;" onclick="loadTemplate('best','none'); return false;">베스트</a></li>
							</ul>
						</div>
						<!-- 
...
<%-- 					<c:set var="baseLink" value="?ctgrId=${c.ctgrId}&page=1"/> --%>
			...
			<div class="pd_list" id="dvdList"></div>
	        <div id="dvdPaging"></div>				
		</div>
	</div>
	<!--// content -->
