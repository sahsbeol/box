...
<ul class="txt_list">
    ...
    <li>
        <strong>이동통신사: </strong>
        <c:if test="${prdtNotiInfo.etc1 eq 'S'}">STK</c:if>
        <c:if test="${prdtNotiInfo.etc1 eq 'K'}">KT</c:if>
        <c:if test="${prdtNotiInfo.etc1 eq 'L'}">U+(LG)</c:if>
        <c:if test="${prdtNotiInfo.etc1 ne 'S' && prdtNotiInfo.etc1 ne 'K' && prdtNotiInfo.etc1 ne 'L'}">${prdtNotiInfo.etc1}</c:if>
    </li>
    <li><strong>이동통신사가입조건: </strong>${prdtNotiInfo.caution}</li>
    <li><strong>소비자 부담사항: </strong>${prdtNotiInfo.addPrice}</li>
