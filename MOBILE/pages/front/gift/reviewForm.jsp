...
function submitFrm(){
    if($("[name=prdtQnaTitle]").val().trim() == ""){
        alert("상품후기를 적어주세요.");
        return false;
    }
    
    
    var byteSize = String($('textarea[name=prdtQnaTitle]').val()).length;
    if(byteSize > 500) {
    	alert("상품후기 내용을 줄이세요.");
        return false;
    }
    ...
    if(byteSize < 15) {
        alert("상품후기는 15글자 이상 적어주세요.");
        return false;
    }
     
    
    var prdtGbn = $("[name=prdtGbn]").val();
    var gradeInt = ""; 
    	if(prdtGbn != "R" && prdtGbn != "D"){
    		gradeInt = parseInt((parseInt($("[name=practicality]").val()) + parseInt($("[name=design]").val()))/2);
    	}else{
    		gradeInt = parseInt($("[name=practicality]").val());
    	}	
...
function findImageFile(){
    ...
    if(/android/i.test(userAgent)){
    	...
    	try{
    		document.location = "myScheme://uploadImage";
    	}catch(err){
    		alert(err);
    	}
    }else{
...
function setFilePath(path){
    if(window.HybridApp != undefined && window.HybridApp.openCrop != undefined){
        $("[name=imageUrl]").val(path);
        $("#thums").attr("src", "http://image.kyobobook.co.kr/newimages/giftshop_new/work/"+path);
    }else{
...
function checkLength(chkObj, printObj, maxLength){
    ...
    if(str.length != 0){
        for(var i = 0; i < str.length; i++){
            if(escape(str.charAt(i)).length > 4)    _byte += 1;
            else                                    _byte++;

            if(_byte > maxLength){
                alert('제한 글자 수를 초과하였습니다');
                $(chkObj).val(str.substr(0,500));
    ...
    $(printObj).html("("+$(chkObj).val().length+"/500)");
}
</script>
</head>
<body>
<!-- content -->
<div id="sc_content" style="margin-top: -10px;">
<!-- tit_section -->
<%-- <div id="sc_header">
    <c:choose>
...
<form name="reviewForm" action="/m/gift/detail/submitReview" method="post">
    ...
    <div class="rating_box">
        ...
        <input type="hidden" name="prdtGbn" value="${product.sellPrdtGbn}"/>
        <c:choose>
    ...
    <p class="info mt5 t_c">단순문자 및 기호의 나열/반복 입력 시 적립금 지급 제외</p>
    <p class="info mt5 t_c">1천원 미만 상품의 경우 적립금 지급제외</p>
    <p class="info mt5 t_c">한 주문 건에 동일 상품을 다수 주문한 경우, 해당 상품에 대한 후기는 1회 작성가능</p>
    <p class="info mt5 t_c">무관한 포토 후기 업로드 시 적립금 지급 제외 될 수 있습니다.</p>
    <p class="info mt5 t_c">우수 상품평을 작성해주신 분께는 매월 100분 추첨하여 25% 쿠폰을 드립니다.</p>
    ...
    <div class="textarea_box"><textarea class="textarea" title=" 상품후기 입력" style="height:100px;" name="prdtQnaTitle" onkeydown="checkLength('textarea[name=prdtQnaTitle]', 'span#chkByte', 501)" onfocusout="checkLength('textarea[name=prdtQnaTitle]', 'span#chkByte', 500)"><c:out value="${ext.prdtQnaTitle}" escapeXml="true"/></textarea></div>
