...
String Okurl        = "https://m.hottracks.co.kr/pages/front/order/lgDacom/payMobilres.jsp";       //[ 128byte 이하] 결제 완료 후 사용자에게 보여질 가맹점측 완료 페이지. (예: http://www.mcash.co.kr/okurl.jsp)
String Prdtnm       = "";       //[  50byte 이하] 상품명
String Siteurl      = "m.hottracks.co.kr";       //[  20byte 이하] 가맹점도메인 (예: www.mcash.co.kr)
...
String CALL_TYPE    = "SELF";      //[   4byte 이하] 결제창 호출 방식 (P: 팝업-default, SELF: 페이지전환, I: 아이프레임)
...
String Closeurl         = "https://m.hottracks.co.kr/pages/front/order/lgDacom/payMobilresFail.jsp";   //[ 128byte 이하] 결제창 취소버튼, 닫기버튼 클릭 시 호출되는 가맹점 측 페이지. iframe 호출 시 필수! (예: http://www.mcash.co.kr/closeurl.jsp)
String Failurl          = "https://m.hottracks.co.kr/pages/front/order/lgDacom/payMobilresFail.jsp";   //[ 128byte 이하] 결제 실패 시 사용자에게 보여질 가맹점 측 실패 페이지. 결제처리에 대한 실패처리 안내를 가맹점에서 제어해야 할 경우만 사용.
...
<input type="hidden" name="Prdtnm" id="Prdtnm" size="30" value="${mobilians.lgdProductInfo}">
<input type="hidden" name="Prdtprice" id="Prdtprice" size="30" value="${mobilians.lgdAmount}">
...
<input type="hidden" name="Tradeid" id="Tradeid" size="50" value="${mobilians.lgdOid}">
...
<!-- 
<input type="button" value="결제하기" onclick="payRequest();">
 -->
</form>
