...
<meta property="og:url" content="http://m.hottracks.co.kr/m/hot/heart" />
...
jQuery(function($){
    ...
    $("#listHeart .thumb_list").scroll(function(){
        _checkMore();
    });
});
...
function _checkMore(){
    ...
    if(obj.scrollLeft() > 0)    $("#listHeart .btn_slide.left").show();
    else                        $("#listHeart .btn_slide.left").hide();
    if(obj.scrollLeft() < max)  $("#listHeart .btn_slide.right").show();
    else                        $("#listHeart .btn_slide.right").hide();
}

function _moreHeart(direct){
    var obj = $(".thumb_box01 .thumb_list");
...
<div class="w_top event_sub_info">
    ...
    <div class="sns">
        <a href="javascript:goTwitter('${detail.eventTitle}', 'http://m.hottracks.co.kr/m/hot/heart')" class="ic_twitter">트위터</a>
        <a href="javascript:goFaceBook('${detail.eventTitle}', 'http://m.hottracks.co.kr/m/hot/heart')" class="ic_facebook">페이스북</a>
        <a href="javascript:sendKakao('[핫트랙스] ${detail.eventTitle}', 'http://m.hottracks.co.kr/m/hot/heart')" class="ic_kakaotalk">카카오톡</a>
    </div>
</div>
<div class="event_info">
    ...
    <div style="display:block;">
        ...
        <div class="thumb_box01" id="listHeart">
            <div class="thumb_title">다른 핫트뿅뿅 보기</div>
            <a href="#" onclick="_moreHeart('L');return false;" class="btn_slide left">왼쪽더보기</a>
            <div class="thumb_list">
                <c:forEach items="${listEvent}" var="l">
                    <a href="/m/hot/heart?eventId=${l.eventId}"><c:if test="${l.curEventYn}"><em class="new">NEW</em></c:if><ui:image src="${l.thumUrl}"/></a>
                </c:forEach>
            </div>
            <a href="#" onclick="_moreHeart('R');return false;" class="btn_slide right">오른쪽더보기</a>
            <a href="/m/hot/heartMain" class="btn_more">더보기</a>
        </div>
    </div>
</div>
<!-- 하트뿅뿅 코멘트 -->
