...
function closeSetting(){
	document.location.href = '${defaultHost}/m/welcomeMain';
/*	
...
function syncData(){
    ...
    var altCouponYn = $("#btnAltCouponYn").hasClass("active")?"Y":"N";
...
function applogout() {
	if($("#btnAutoLoginYn").hasClass("active")){
		$("#btnAutoLoginYn").removeClass("active");
		syncData();
	}
	logout();
}
...
<div id="ly_setting">
        ...
        <c:choose>
            <c:when test="${not empty user}">
                <div class="info_box">
                    <strong>로그인정보</strong>
                    ${user.userId}
                    <a href="#" onclick="applogout();return false;" class="btn bg_red">로그아웃</a>
                </div>
            </c:when>
            <c:otherwise>
                <div class="box_flex">
                    <a href="#" onclick="login();return false;" class="btn bg_red">로그인</a>
                </div>
            </c:otherwise>
        </c:choose>
    </div>
    ...
    <div class="radio_box">
        <span>쿠폰혜택 알림 받기</span>
        <a href="#" id="btnAltCouponYn"<c:if test="${device.altCouponYn ne 'N'}"> class="active"</c:if>>쿠폰혜택 알림 받기</a>
    </div>
    <h4>
    ...
    <div class="version_box">
        <c:if test="${not empty rcntVersion && not empty device && device.devGbnCode eq 'C1331' && device.devVersion ne rcntVersion}">
            <div class="box_flex">
                <a href="#" onclick="goAppUpdate();return false;" class="btn bg_red">최신버전 업데이트</a>
            </div>
        </c:if>
        <p class="txt">
            <strong>현재버전</strong>
            <em>${device.devVersion}</em>
            <c:if test="${not empty rcntVersion && not empty device && device.devGbnCode eq 'C1331'}">
                <strong>최신버전</strong>
                <em class="col_orange">${rcntVersion}</em>
            </c:if>
        </p>
    </div>
    <a href="javascript:closeSetting();" class="btn_cls">닫기</a>
</div>
</body>
</html>