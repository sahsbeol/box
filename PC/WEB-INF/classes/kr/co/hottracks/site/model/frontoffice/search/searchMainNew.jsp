...
jQuery(function($){
    ...
    $("form[name=searchedForm]").submit(function() {
        ...
        if($.trim($("form[name=categoryBrandCount]").val()) == "")    $.trim($("form[name=categoryBrandCount]").val(0);
    ...
    $("form[name=searchedForm] input[name=searchTerm]").focusout(function(){
...
<body>
<form name="searchedForm" method="post" action="/ht/search/searchMainNew">
    ...
    <div id="searchProduct" class="mgt30">
        <c:if test="${empty result.listProduct}">
            <c:if test="${empty listEvent}"><p style="text-align:center;"><img src="${imageServer}/images/renewal/shop/img_no_result02.jpg" alt="죄송합니다. 고객님께서 찾으시는 상품이 없습니다.ㅠㅠ"/></p></c:if>
            <c:if test="${not empty listEvent}"><p style="text-align:center;"><img src="${imageServer}/images/renewal/shop/img_result_onlyevent02.jpg" alt="죄송합니다. 고객님께서 찾으시는 상품이 없네요."/></p></c:if>
        </c:if>
                            ...
                            <c:if test="${fn:startsWith(l.ctgrId, '0000')}">
                                <li>
                                    ...
                                    <a href="/ht/product/detail?barcode=${l.sellPrdtBcode}">
                                        ...
                                        클릭수 : <fmt:formatNumber type="number">${l.clickCount}</fmt:formatNumber>
                                        </span>
                                        <span class="name">${l.prdtName}</span>
                                        <span class="icon">
                                            <c:if test="${l.reviewCount > 0}"><img src="${imageServer}/images/renewal/content/ico_review01.jpg" alt="후기"/></c:if>
                                            <c:if test="${l.wishCount > 0}"><img src="${imageServer}/images/renewal/content/ico_like02.jpg" alt="좋아요"/></c:if>
