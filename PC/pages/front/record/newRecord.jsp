...
<head>
<title>
<c:if test="${fn:startsWith(c.ctgrId, '0003')}">새로나온 CD</c:if>
<c:if test="${fn:startsWith(c.ctgrId, '0004')}">새로나온 BLU-RAY/DVD</c:if>
- HOTTRACKS
</title>
    ...
    <c:if test="${fn:startsWith(c.ctgrId, '0003')}">
        ...
        <ul>
            <c:forEach items="${categoryListLeft}" var="l" varStatus="st">
                <li><a href="/ht/record/newRecord/${l.cateId}" <c:if test="${c.ctgrId eq l.cateId}">class="on"</c:if>>${l.cateName}</a></li>
            </c:forEach>
        </ul>
    </c:if>
