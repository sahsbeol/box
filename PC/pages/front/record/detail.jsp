...
<meta property="eg:itemId" content="${p.sellPrdtBcode}" />
<meta property="eg:itemName" content="${p.commTitle}" />
<meta property="eg:itemImage" content="${productImageUrl}" />
<meta property="eg:itemUrl" content="${defaultHost}/ht/record/detail/${p.sellPrdtBcode}" />
<meta property="eg:originalPrice" content="${p.prdtSellPrice}" />
<meta property="eg:salePrice" content="${p.lastCpnPrice}" />

<c:set var="stind" value="0"/>
<c:forEach varStatus="st" begin="4" end="${fn:length(p.ctgrId)}" step="2">
    <c:set var="stind" value="${stind+1}"/>
    <meta property="eg:category${stind}" content="${fn:substring(p.ctgrId, 0, st.index)}" />
</c:forEach>

<meta property="eg:brandId" content="" />
<meta property="eg:brandName" content="" />
<meta property="eg:regDate" content="${p.regDate}" />
...
<c:forEach varStatus="st" begin="6" end="${fn:length(p.ctgrId)}" step="2">
	<script type="text/javascript">
		ctgrPath.push('<ui:category id="${fn:substring(p.ctgrId, 0, st.index)}" full="false"/>');
	</script>
</c:forEach>
...
_n_p2=ctgrPath[0]; 			// 중카테고리명
_n_p3=ctgrPath[1]; 			// 소카테고리명 
_n_p4=ctgrPath[2]; 			// 세카테고리명
_n_p5='${p.commTitle}'; 			// 상품명 
</script>
<!-- 데이타 스토리  script end-->
...
function setSlide(){
    swiper = new Swiper('.best_music .swiper-container',{
        nextButton: '.best_music .swiper-button-next',
        prevButton: '.best_music .swiper-button-prev',
        pagination: '.best_music .swiper-pagination',
...
var _rbSendSyncLogs = function() {
	...
	eglc.op('setVar','itemId','${p.sellPrdtBcode}');
    ...
    <div class="inner">
        ...
        <div class="detail_cont">
                ...
                <ul class="mediainfo">
                    <c:if test="${p.sellPrdtGbn eq 'R'}">
                        <c:if test="${not empty p.artiName}">
                            <li><strong>아티스트</strong> : <a href="#" onclick="goSearchDtl('${p.artiName}', 'label_name', 'RECORD');return false;">${p.artiName}</a> <span class="line">|</span></li>
                        </c:if>
            ...
            <span class="ico_label badge">
                ...
                <!-- 지관통포스터 추가 -->
                <c:if test="${p.hasPosterCase}"><span class="badge01">지관통포스터</span></c:if>
                    ...
                    <li>
                        ...
                        <div class="price">
                            ...
                            <c:if test="${p.prdtSellPrice > p.lastDcPrice}">
                                ...
                                <strong><fmt:formatNumber value="${p.lastDcPrice}" pattern=",###" /></strong> (<fmt:parseNumber var="percent" integerOnly="true" value="${hfn:rate(p.prdtSellPrice,p.lastDcPrice)}"/>${percent}%)
<%--                                  (${hfn:rate(p.prdtSellPrice, p.lastDcPrice)}%) --%>
                            </c:if>
                        </div>
                    </li>
                ...
                <div class="btn_wrap mgt35<c:if test="${not empty p.offlineUrl}"> small01</c:if>">
                    ...
                    <c:if test="${p.saleYn eq 'Y'}">
                        ...
                        <a href="#" onclick="toggleWichProduct();return false;" class="btn_type04_v2 mgl7" style="width:133px;">
                        	<em class="btn_like ${linkCls}">
                        		<c:choose>
							        <c:when test = "${p.wishPrdtCnt > 999}">
							        	999+ 
							        </c:when>
							        <c:otherwise>
							         	${p.wishPrdtCnt}
                    ...
                    <c:if test="${p.outOfStockMailYn eq 'Y'}">
                        <a href="#" onclick="addRecordStockedInfoRequest('${p.rcrdCd}');" class="btn_type04_v2">알림예약 신청하기</a>
                        <a href="#" onclick="toggleWichProduct();return false;" class="btn_type04_v2 mgl7" style="width:133px;">
	                        <em class="btn_like ${linkCls}">
	                        	<c:choose>
							        <c:when test = "${p.wishPrdtCnt > 999}">
...
<div class="detail_p_info">
    <!-- 음반정보 -->
    <div class="" id="detail_cont01" style="padding-top:140px;">
        <ul class="detail_tab">
    ...
    <div style="padding-top:140px;" id="detail_cont03">
        <ul class="detail_tab">
            <li><a href="#detail_cont01">${p.sellPrdtGbn eq 'R'?'음반':'상품'}정보</a></li>
            <li>
            	<a href="#detail_cont02">상품후기<span class="product-estimation c_salmon mgl10">(0)</span></a>
            </li>
