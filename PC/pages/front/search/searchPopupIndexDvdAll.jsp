...
<div class="pop_cont">
    <form name="form" method="post" action="/ht/search/searchPopupIndex/DvdAll">
    ...
    <div class="search_index">
        <div class="rdarea">
            <input type="radio" id="index1" name="index_finder" value="drctr"  <c:if test="${fn:toLowerCase(fn:substring(criteria.subMenu, 6, fn:length(criteria.subMenu) ) ) eq 'drctr'}">checked</c:if> /><label for="index1">감독</label>
            <input type="radio" id="index2" name="index_finder" value="actor" <c:if test="${fn:toLowerCase(fn:substring(criteria.subMenu, 6, fn:length(criteria.subMenu) ) ) eq 'actor'}">checked</c:if> /><label for="index2">배우/츨연진</label>
        </div>
