...
<div class="pop_cont">
    <form name="form" method="post" action="/ht/search/searchPopupIndex/DvdClassic">
    ...
    <div class="search_index">
        <div class="rdarea">
            <input type="radio" id="index1" name="index_finder" value="cmpsr"  <c:if test="${fn:toLowerCase(fn:substring(criteria.subMenu, 6, fn:length(criteria.subMenu) ) ) eq 'cmpsr'}">checked</c:if> /><label for="index1">작곡가</label>
            <input type="radio" id="index2" name="index_finder" value="cndtr" <c:if test="${fn:toLowerCase(fn:substring(criteria.subMenu, 6, fn:length(criteria.subMenu) ) ) eq 'cndtr'}">checked</c:if> /><label for="index2">지휘자</label>
            <input type="radio" id="index3" name="index_finder" value="prfmr_kor" <c:if test="${fn:toLowerCase(fn:substring(criteria.subMenu, 6, fn:length(criteria.subMenu) ) ) eq 'prfmr_kor'}">checked</c:if> /><label for="index3">연주자</label>
            <input type="radio" id="index4" name="index_finder" value="vocal" <c:if test="${fn:toLowerCase(fn:substring(criteria.subMenu, 6, fn:length(criteria.subMenu) ) ) eq 'vocal'}">checked</c:if> /><label for="index4">보컬</label>
            <input type="radio" id="index5" name="index_finder" value="ocstr" <c:if test="${fn:toLowerCase(fn:substring(criteria.subMenu, 6, fn:length(criteria.subMenu) ) ) eq 'ocstr'}">checked</c:if> /><label for="index5">오케스트라</label>
            <input type="radio" id="index6" name="index_finder" value="label_name" <c:if test="${fn:toLowerCase(fn:substring(criteria.subMenu, 6, fn:length(criteria.subMenu) ) ) eq 'label_name'}">checked</c:if> /><label for="index6">레이블</label>
        </div>
