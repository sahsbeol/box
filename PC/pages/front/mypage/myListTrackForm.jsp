...
function add(folderSeq){
    $.ajax({
        async : true, 
        url : "/ht/mypage/myListTrackAdd",
        dataType : "json", 
        type : "post", 
        data : {
    	   myListFolderSeq : folderSeq, 
    	   trkIdList : "${c.trkIdList}" 
        }, 
        success : function(result, textStatus, XMLHttpRequest){
            if(result.errorMessages.length == 0){
                location.href = "/ht/mypage/myListTrackAddEnd?folderSeq="+folderSeq;
            }else{
...
function addFolderCreate(){
    ...
    $.ajax({
        async : true, 
        url : "/ht/mypage/myListTrackAddByFolder",
        dataType : "json", 
        type : "post", 
        data : {
        ...
        success : function(result, textStatus, XMLHttpRequest){
            if(result.errorMessages.length == 0){
            	location.href = "/ht/mypage/myListTrackAddEnd";
            }else{
                alert(result.errorMessages[0]);
            }
        }, 
...
function goListView(myListFolderSeq){
	if(opener.opener) {
		opener.opener.location.href = "/ht/mypage/myListTrackList?myListFolderSeq="+myListFolderSeq;
	} else {
		opener.location.href = "/ht/mypage/myListTrackList?myListFolderSeq="+myListFolderSeq;
	}
	window.close();
}
    ...
    <div class="pop_body" style="height:420px;">
        <div class="box_addfavor">
            ...
            <table>
                ...
                <thead>
                    <tr>
                        <th scope="col" colspan="4">담은 음원 폴더명</th>
                    </tr>
                </thead>
                <tbody>
            ...
            <div class="add_folder">
                ...
                <p>
                                    폴더는 50개까지 생성 가능합니다.<br />
                                    한 폴더당 100곡까지 담으실 수 있습니다.
                </p>
            </div> 
        </div>
    </div> 
