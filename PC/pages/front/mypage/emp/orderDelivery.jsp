...
<div class="sub_contents">
    ...
    <c:if test="${not empty sellPrdtBcode}">
        ...
        <c:if test="${empty result || empty result.stInt1 }"><span class="c_orange li18">※ 구매하지 않은 상품이거나 구매 후 3개월이 지난 상품</span>입니다. 상품을 구매하신 뒤 다시 상품평을 작성해주세요.</c:if>
        <c:if test="${not empty result && result.stVar1 ne 'C0124' }"><span class="c_orange li18">※ [ ${result.stVar2 } ]</span> 상품은 배송완료 상태인 경우에 상품평을 작성 하실 수 있습니다.</c:if>
        </div>
    </c:if>
