...
function save(){
    ...
    var emailEx1 = /^[-A-Za-z0-9_]+[-A-Za-z0-9_.]*[@]{1}[-A-Za-z0-9_]+[-A-Za-z0-9_.]*[.]{1}[A-Za-z]{2,5}$/;
    
    if($("input[name=emailAddr1]").val() == "" ||  $("input[name=emailAddr2]").val() == ""){
        alert("메일주소를 정확히 입력해 주시기 바랍니다.");
        return;
    }else if(($("input[name=emailAddr1]").val() + "@" + $("input[name=emailAddr2]").val()).search(emailEx1) == -1){
        alert("메일주소를 형식이 올바르지 않습니다. (한글,영문,특수문자'_'만 가능합니다.)");
        $("input[name=emailAddr1]").focus();
        return;
    }else{
    ...
    $.ajax({
        async : true, 
        url : "/ht/mypage/recordStockedInfoRequestSubmit",
        dataType : "json", 
        type : "post", 
        data : data, 
...
<div style="">
<form id="createForm">
<input type="hidden" name="rcrdCd" value="${c.rcrdCd}" />
...
<div class="pop_cont">
    ...
    <ul class="list_type02 mgt30">
        ...
        <li class="point01">E-mail/SMS 수신거부 상태에서는 알림 메일을 받으실 수 없습니다. <a href="javascript://" onclick="cock();" class="btn_type10 f_right"><span>회원정보 수정</span></a></li>
    </ul>
</div>
<div class="pop_close">
