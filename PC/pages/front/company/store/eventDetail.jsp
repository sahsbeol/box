...
<div id="contents_area">
    ...
    <div id="location">
        ...
        <em class="location"><img src="http://image.hottracks.co.kr/info/icon/icon_home.png"  alt="홈"/> &gt; 영업점 &gt; ${store.deptname} &gt; <strong>행사안내</strong></em>
    </div>
    <!-- //location -->
    <div class="table_view">
        ...
        <p class="rightBtnView"><a href="/company/store/event?deptcode=${c.deptcode}&page=${c.page}&srchGbn=${c.srchGbn}&srchKeyword=${c.srchKeyword}" title="목록보기"><img src="http://image.hottracks.co.kr/info/btn/btn_pre_list.png" alt="목록보기"/></a></p>
    </div>
        ...
        <table summary="순번, 행사명, 행사기간으로 구성 된 지점별 이벤트 게시판" cellpadding="0" cellspacing="0">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="Tleft">
                        <a href="/company/store/eventView?deptcode=${c.deptcode}&boardseq=${l.boardseq}" eventlist="eventlist" boardseq="${l.boardseq}">${l.title }</a><c:if test="${l.newYn eq 'Y'}"><img src="http://image.hottracks.co.kr/info/common/icon_new.png" alt="New" /></c:if>
                    </td>
                    <td>
    ...
    <!-- paging -->
    <div class="paging">
        <ui:pagination name="pageHolder" url="/company/store/eventView" parameters="srchGbn,srchKeyword,deptcode,boardid,boardseq"/>
    </div>
    <!-- //paging -->
    <!-- search -->
    <form method="post" action="/company/store/event" onsubmit="return true" name="frmSearch" id="frmSearch">
        <input type="hidden" name="storId" value="${c.deptcode}" />
