        ...
        <table summary="행사안내 상세보기 테이블 행사명, 진행영업점, 행사기간으로 구성 된 행사안내 상세보기" cellpadding="0" cellspacing="0">
            ...
            <colgroup>
                ...
                <tbody>
                    ...
                    <tr><td colspan="4" class="view_area_left"></td></tr>
                </tbody>
        </table>
        <p class="rightBtnView"><a href="/company/story/notice?deptcode=${c.deptcode}&page=${c.page}&srchGbn=${c.srchGbn}&srchKeyword=${c.srchKeyword}" title="행사안내 목록보기"><img src="http://image.hottracks.co.kr/info/btn/btn_pre_list.png" alt="목록보기" id="list"/></a></p>
    </div>
    <!-- 리스트(공통) -->
        ...
        <table summary="공지사항 리스트 테이블입니다. 번호, 제목, 등록일, 조회수로 구성 된 공지사항" cellpadding="0" cellspacing="0">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="Tleft"><a href="/company/story/noticeView?boardseq=${l.boardseq}" title="${l.title }" list="list" boardseq="${l.boardseq}">${l.title}</a></td>
        ...
        <div class="searchWrap">
            <p>
                <select name="srchGbn" title="전체 제목 내요 선택">
                    <option value="all" <c:if test="${c.srchGbn eq 'all'}">selected="selected"</c:if>>전체</option>
                    <option value="title" <c:if test="${c.srchGbn eq 'title'}">selected="selected"</c:if>>제목</option>
                    <option value="contents" <c:if test="${c.srchGbn eq 'contents'}">selected="selected"</c:if>>내용</option>
                </select>
