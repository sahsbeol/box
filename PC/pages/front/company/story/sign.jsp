...
<div id="contents_area">
                    ...
                    <ul class="bannerList">
                        <c:forEach items="${list}" var="l" varStatus="st" begin="${s1.index}" end="${s1.index+2}">
                            <li <c:if test="${st.first}">class="firstChild"</c:if>>
                                <div class="bannerImg">
                                    <a href="/company/story/signView?boardseq=${l.boardseq}" title="${l.title}"><img width="209" height="77" src="http://info.hottracks.co.kr${l.imgurl}" alt="${l.title }" /></a>
                                    <c:choose>
                                ...
                                <ul class="bannerTxt">
                                    ...
                                    <li>
                                        <c:set var="deptnames" value="${fn:replace(l.deptnames,'/',',') }"/>
                                        <c:set var="arrDpCdName" value="${fn:split(deptnames, ',') }"/> 
                                        <c:choose>
    ...
    <!-- paging -->
    <div class="paging">
        <ui:pagination name="pageHolder" url="/company/story/sign" parameters="srchGbn,srchKeyword"/>
    </div>
    <!-- //paging -->
    <!-- search -->
