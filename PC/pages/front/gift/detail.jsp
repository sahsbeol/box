...
<c:forEach varStatus="st" begin="6" end="${fn:length(category.cateId)}" step="2">
	<script type="text/javascript">
		ctgrPath.push('<ui:category id="${fn:substring(category.cateId, 0, st.index)}" full="false"/>');
	</script>
</c:forEach>
...
<script type="text/javascript">
	...
	jQuery(function($) {
	    ...
	    $(document).ready(function(){
	        ...
	        $('.t_tab > .tab').on('mouseover', function() {
	...
	function setSlide(){	
        swiper = new Swiper('.best_gift .swiper-container',{
            nextButton: '.best_gift .swiper-button-next',
            prevButton: '.best_gift .swiper-button-prev',
            pagination: '.best_gift .swiper-pagination',
                                ...
                                <c:choose>
                                    <c:when test="${ext.prdtSellPrice ne ext.lastDcPrice}">
                                        <span class="ori_price"><fmt:formatNumber value="${ext.prdtSellPrice}" type="number" /></span>
                                         <strong><fmt:formatNumber value="${ext.lastDcPrice}" type="number" /></strong> (<fmt:parseNumber var="percent" integerOnly="true" value="${hfn:rate(ext.prdtSellPrice,ext.lastCpnPrice)}"/>${percent}%)
                                    		<%-- <strong><fmt:formatNumber value="${ext.lastDcPrice}" type="number" /></strong> (<fmt:parseNumber var="percent" integerOnly="true" value="${hfn:rate(ext.prdtSellPrice,ext.lastDcPrice)}"/>${percent}%) --%>
<%--                                     		${hfn:rate(ext.prdtSellPrice, ext.lastDcPrice)}%) --%>
                                    </c:when>
                                    <c:otherwise>
                                        ...
                                        <div class="layer_pop_container" style="width:309px;line-height:20px;">
                                            ...
                                            <div class="layer_pop_cont pd0">
                                                <ul class="card_list">
                                                    ...
                                                    <%-- <li>
	                                                    <span class="img"><img src="${imageServer}/images/common/sscard1_80_50.png" alt="삼성카드"/></span>
                                                        <div class="txt">
                                                            <strong>※삼성카드 3만원 이상 결제 시<br/>2,500원 즉시할인</strong>
                                                            -이벤트 기간 : 2020.03.16 ~ 소진 시<br/>
                                                            -신용카드 결제 창에서 쿠폰 적용이 된 경우 해당되며, 쿠폰 적용이 되지 않을 경우 조건에 부합하지 않거나, 쿠폰 수량이 모두 소진된 경우입니다.
                                                        </div>
                                                    </li> --%>
                                                </ul>
                                            </div>
                    ...
                    <div class="btn_wrap mgt35">
                        ...
                        <c:if test="${salability && ext.saleStat ne 'N'}">
                            ...
                            <a href="#" onclick="toggleWichProduct();return false;" class="btn_type04_v2 fs18 mgl7" style="width:133px;">
                            	<em class="btn_like ${linkCls}">
                           			<c:choose>
                        ...
                        <c:if test="${!salability or ext.saleStat eq 'N'}">
                            ...
                            <c:if test="${dlvyVendor.vendorId ne '0625'}">
                                <c:if test="${limitProduct eq null || limitProduct.limitCount - limitProduct.laveCount > 0}">
                                <a href="#" onclick="return false;" class="btn_dim btn_type04_v1 fs18" style="width:133px;">바로구매</a>
                                </c:if>
                                <c:if test="${limitProduct eq null}">
...
<div class="best_gift_a">
	<ul class="best_gift">
	    <c:forEach begin="0" end="2" varStatus="s1">
	        ...
	        <c:if test="${s1.index eq 2}"><c:set var="title" value="브랜드 연관 상품"/><c:set var="list" value="${listRecobell3}"/><c:set var="link" value="/ht/hot/brandDetail?brandId=${ext.brandId}&productOrderBy=P"/></c:if>
	        <c:if test="${not empty list}">
	            	<li class="t_tab<c:if test="${s1.first}"> active</c:if>">
	                <a href="javascript://" class="tab"><span>${title}</span></a>
	                <div class="depth2 swiper-container" data-index="${s1.index}">
			                        ...
			                        <c:forEach items="${list}" var="l" varStatus="st" begin="${s.index}" end="${s.index+3}">
			                            <li class="txt_line1">
			                            	<c:if test="${l.sellPrdtGbn eq 'R' || l.sellPrdtGbn eq 'D'}">
			                            		<a href="/ht/record/detail/${l.sellPrdtBcode}?foo=bar&rccode=<c:if test="${s1.index eq 0}">pc_detail1</c:if><c:if test="${s1.index eq 1}">pc_detail2</c:if><c:if test="${s1.index eq 2}">pc_detail3</c:if>" title="${l.prdtName}">
			                            	</c:if>
				                            <c:if test="${l.sellPrdtGbn eq 'P' || l.sellPrdtGbn eq 'G' || l.sellPrdtGbn eq 'S'}">
				                            	<a href="/ht/product/detail?barcode=${l.sellPrdtBcode}&foo=bar&rccode=<c:if test="${s1.index eq 0}">pc_detail1</c:if><c:if test="${s1.index eq 1}">pc_detail2</c:if><c:if test="${s1.index eq 2}">pc_detail3</c:if>" title="${l.prdtName}">
				                            </c:if>
...
<div class="detail_p_info">
    ...
    <div class="" id="detail_cont01">
        ...
        <c:if test="${not empty bnrPrdtDetail}">
            ...
            <c:if test="${ext.sellPrdtBcode eq '2300186032559'}">
            	<p style="text-align:center;margin:20px 0;">
	                <a href="/e/70114"><img src="http://image.kyobobook.co.kr/newimages/giftshop_new/work/1368/750_250_2.jpg" style="max-width:100%;"/></a>
                </p>
            </c:if>    
        </c:if>
    ...
    <div style="padding-top:140px;" id="detail_cont02">
        ...
        <div class="tbl_top mgt0">
            <p class="txt_l">이 상품, 어떠셨나요? 상품 구입 후 후기를 남겨주시면, 기본 100P / 포토후기 200P 를 적립해 드립니다.</p>
            <div class="f_right">
