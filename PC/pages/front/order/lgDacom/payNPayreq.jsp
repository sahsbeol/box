...
function payRequest(){
    //alert('${nReserveId}');
//    newwin = window.open('https://alpha2-pay.naver.com/payments/${nReserveId}', '_blank', 'width=750,height=1000,scrollbars=yes');
    newwin = window.open('https://pay.naver.com/payments/${nReserveId}', '_blank', 'width=750,height=1000,scrollbars=yes');
    ...
    loop = setInterval(function() {   
        if(newwin.closed && naverNPayReq == 0) {  
            clearInterval(loop);  
...
function payReturn(resultCode,paymentId,resultMessage,reserveId) {
	// resultCode : Success -> 결제 성공 , userCancel -> 구매자 취소 , webhookFail -> webhookUrl 호출응답 실패 , paymentTimeExpire -> 결제시간 초과
	//alert("결제리턴코드:" + resultCode + "/paymentId:" + paymentId + "/resultMessage:" + resultMessage + "/reserveId:" + reserveId);
	...
	document.forms['orderInfoForm'].nPayPaymentId.value = paymentId;
...
<form id="orderInfoForm" action="#" method="post">
    ...
    <input type="hidden" name="nPayPaymentId" value=""/>
