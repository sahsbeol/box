    ...
    <tbody id="tbodyQna">
        ...
        <c:forEach items="${list}" var="dto" varStatus="status">
            <tr>
                ...
                <td class="t_left"><a href="#qna-content0${status.index + 1}" class="click">${hfn:lineToBr(dto.title)}</a></td>
                <td>
                    <c:if test="${isLogin && dto.userNum eq currentLoginUserNum}"> 
                        <input  type="hidden" name="seq" value="${dto.seq}" />
                        <c:if test="${dto.answerYn eq 'N'}">
            ...
            <tr class="conts" id="qna-content0${status.index + 1}">
                <td colspan="6" class="detail-content">
                    <div class="txt">
                        <p class="question-content">${hfn:lineToBr(dto.content)}</p>
                        <c:choose>
                            <c:when test="${dto.answerYn eq 'Y'}">
                                ...
                                <p class="answer-content">${hfn:lineToBr(dto.answerContent)}</p>
                            </c:when>
                            <c:otherwise>
