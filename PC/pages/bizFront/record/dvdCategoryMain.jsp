...
$(document).ready(function() {
	...
	if("2" == cateDepthInfo){
		loadRecordTemplate("preOrder","dvd");		// 예약상품
		loadRecordTemplate("newReleases","dvd");		// 새로 나온 상품	
	}
...
// 예약상품 [가요/팝/클래식] 조회
function loadRecordTemplate(listGbn, genre){	// listGbn : [예약상품, 새로나온 상품], genre : [dvd]   
...
<div class="lower_content">	
	...
	<div class="location_wrap">
		...
		<c:if test="${not empty cate1DepthList}">
			<div class="location_nav_wrap">
				<a href="/ht/biz/record/dvdCategoryMain?ctgrId=${cate1DepthLoc.cateId}" class="location_1 <c:if test="${cateDepthInfo eq '1'}">on</c:if>">${cate1DepthLoc.cateName}</a>
		...
		<c:if test="${not empty cate2DepthList}">
			<div class="location_nav_wrap">
				<a href="/ht/biz/record/dvdCategoryMain?ctgrId=${cate2DepthLoc.cateId}" class="location_1 <c:if test="${cateDepthInfo eq '2'}">on</c:if>">${cate2DepthLoc.cateName}</a>
				<ul class="location_nav">	
					<c:forEach items="${cate2DepthList}" var="l" varStatus="st">				
						<li><a href="/ht/biz/record/dvdCategoryMain?ctgrId=${l.cateId}">${l.cateName}</a></li>            		       		
					</c:forEach>
				</ul>	
			</div>		
		</c:if>
		...
		<c:if test="${not empty cate3DepthList}">
			<div class="location_nav_wrap">
				<a href="/ht/biz/record/dvdCategoryMain?ctgrId=${cate3DepthLoc.cateId}" class="location_1 <c:if test="${cateDepthInfo eq '3'}">on</c:if>">${cate3DepthLoc.cateName}</a>
				<ul class="location_nav">	
		...
		<c:if test="${not empty cate4DepthList}">
			<div class="location_nav_wrap">
				<a href="/ht/biz/record/dvdCategoryMain?ctgrId=${cate4DepthLoc.cateId}" class="location_1 <c:if test="${cateDepthInfo eq '4'}">on</c:if>">${cate4DepthLoc.cateName}</a>
				<ul class="location_nav">	
	...
	<c:if test="${cateDepthInfo eq '2'}">
		<div class="sub_tab">
			<ul>
				<li <c:if test="${c.ctgrId eq '000401'}">class="active"</c:if>><a href="/ht/biz/record/dvdCategoryMain?ctgrId=000401">DVD</a></li>
				<li <c:if test="${c.ctgrId eq '000400'}">class="active"</c:if>><a href="/ht/biz/record/dvdCategoryMain?ctgrId=000400">BLU-RAY</a></li>
			</ul>
		</div>
	</c:if>
	<c:if test="${cateDepthInfo ne '2'}">
		...
		<ul>
			...
			<c:if test="${not empty subCateList}">
				<c:forEach items="${subCateList}" var="l" varStatus="st">
					<li><a href="/ht/biz/record/dvdCategoryMain?ctgrId=${l.cateId}" <c:if test="${c.ctgrId eq l.cateId}">style="color:#DA2128;"</c:if>>${l.cateName}</a></li>
				</c:forEach>
			</c:if>		
		</ul>
		<c:if test="${c.ctgrId eq '000400' || c.ctgrId eq '000401'}">
			<div class="type02">
				<ul>
					<li><a href="/ht/record/bestList/000401?bestRangeType=C1002" target="_blank">베스트</a></li>
					<li><a href="/ht/record/newRecord/000401" target="_blank">새로나온상품</a></li>
					<li><a href="/ht/record/reserve/000401" target="_blank">예약상품</a></li>
					<li><a href="/ht/search/searchDetail/Dvd" target="_blank">DVD / BLU-RAY 상세 검색</a></li>							
				</ul>
			</div>		
		</c:if>	
	</div>
	...
	<!-- 베스트 -->
	<c:if test="${empty bestSellers }"><c:set var="bestSellers" value="${bestSellersNone}"/></c:if>
	<c:if test="${not empty bestSellers }">	
		<div class="pd_unit pd_unit04">
			<h3 class="category_tit"><c:if test="${c.ctgrId eq '000401'}"><span class="mgr10">DVD</span></c:if><c:if test="${c.ctgrId eq '000400'}"><span class="mgr10">BLU-RAY</span></c:if>베스트</h3>
			<div class="category_slide dvd">		
				<ul>						
				<c:forEach items="${bestSellers}" var="l" varStatus="st">								
					<!-- 이미지 원형인 경우 round : class 추가 -->
					<li class="<c:if test="${l.saleYn ne 'Y'}"><c:choose><c:when test="${l.rcrdStat eq '3' || l.rcrdStat eq '4'}">sold_out-m</c:when><c:otherwise>sold_out-t-m</c:otherwise></c:choose></c:if>">
						<c:choose>
							<c:when test="${l.saleYn ne 'Y'}">
								<a href="/ht/record/detail/${l.sellPrdtBcode}">
									<div class="thum">
										<ui:image rcrdCd="${l.rcrdCd}" prdtGbn="${l.sellPrdtGbn}" rcrdImgType="large" width="${l.sellPrdtGbn eq 'R'?'145':'95'}" alt="${l.prdtName}" ratingCode="A" />
	...
	<div class="tab">
		<ul class="col05">
			...
			<c:if test="${c.ctgrId ne '000401' && c.ctgrId ne '000400'}">
	...
	<div class="pagination_v2"><ui:bizPagination name="pageHolder" url="/ht/biz/record/dvdCategoryMain?" parameters="ctgrId,productOrderBy,saleYn,freeDlvYn,hotDlvYn,cpYn,dcYn,rsvYn"/></div>
