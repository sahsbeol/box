...
<head>
<title>
${navi[fn:length(navi)-1].nm}
<c:if test="${fn:length(navi) > 1}"> | ${navi[fn:length(navi)-2].nm}</c:if>
<c:if test="${fn:length(navi) > 2}"> | ${navi[fn:length(navi)-3].nm}</c:if>
- HOTTRACKS
</title>
...
<div class="cate03">
    <h2 class="tit">
        <c:if test="${fn:startsWith(c.ctgrId, '0003')}"><img src="${imageServer}/images/renewal/shop/cdsub_tit${fn:substring(c.ctgrId, 0, 6)}.gif" alt="CD" /></c:if>
        <c:if test="${fn:startsWith(c.ctgrId, '000400')}"><img src="${imageServer}/images/renewal/shop/sub_tit10.gif" alt="Blu-Ray" /></c:if>
        <c:if test="${fn:startsWith(c.ctgrId, '000401')}"><img src="${imageServer}/images/renewal/shop/sub_tit11.gif" alt="DVD" /></c:if>
    </h2>
    <ul>
        <c:forEach items="${categoryListLeft}" var="l" varStatus="st">
            <li><a href="/ht/record/category/${l.cateId}" <c:if test="${c.ctgrId eq l.cateId}">class="on"</c:if>>${l.cateName}</a></li>
        </c:forEach>
    </ul>
</div>
...
<div class="list_section">
    ...
    <div class="page_location">
        <div class="loc">
            ...
            <c:if test="${fn:startsWith(c.ctgrId, '0003')}"><a href="/ht/biz/record/recordCategoryMain?ctgrId=0003">CD</a></c:if>
            <c:if test="${fn:startsWith(c.ctgrId, '0004')}"><a href="/ht/biz/record/dvdCategoryMain?ctgrId=000401">DVD</a></c:if>
            <c:forEach varStatus="st" begin="6" end="${fn:length(c.ctgrId)}" step="2">
                <c:if test="${st.last}"><em><ui:category id="${c.ctgrId}" full="false"/></em></c:if>
                <c:if test="${!st.last}">
                    <c:if test="${fn:substring(c.ctgrId, 0, st.index) eq '000303'}"><em><ui:category id="${fn:substring(c.ctgrId, 0, st.index)}" full="false"/></em></c:if>
                    <c:if test="${fn:substring(c.ctgrId, 0, st.index) ne '000303'}"><a href="/ht/record/category/${fn:substring(c.ctgrId, 0, st.index)}"><ui:category id="${fn:substring(c.ctgrId, 0, st.index)}" full="false"/></a></c:if>
                </c:if>
            </c:forEach>
        </div>
    </div>
