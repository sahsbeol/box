...
<html>
<head>
<title><ui:category id="${ctgrId}" full="N"/> | CD - HOTTRACKS</title>
...
<div class="record_cd02 mgb25 mgt50">
    <div class="cate03">
        <h2 class="tit"><img src="${imageServer}/images/renewal/shop/cdsub_tit${fn:substring(ctgrId, 0, 6)}.gif" alt="CD" /></h2>
        <ul>
    ...
    <div class="record_carousel type02">
        <div class="page_location record">
            <div class="loc">
                <a href="/" class="home">Home</a><a href="/ht/biz/record/recordCategoryMain?ctgrId=0003">CD</a><em><ui:category id="${ctgrId}" full="false"/></em>
            </div>
        </div>
        <div class="main_sec7">
...
<div class="pre_orders mgt45 mgb55">
    ...
    <a href="/ht/record/reserve/${ctgrId}" class="btn_more">MORE +</a>
                        ...
                        <c:forEach items="${list5St}" var="l" varStatus="st" begin="${s1.index}" end="${s1.index+4}">
                            <li>
                                <a href="/ht/record/detail/${l.sellPrdtBcode}" title="${l.artiName} - ${l.prdtName}">
                                    ...
                                    <!--  직접전시:유아어린이(000310) / 재즈(000303) / 국악(000309) / 종교음악(000311)  -->
                                    <c:if test="${l.ctgrId eq '000310' || l.ctgrId eq '000303' || l.ctgrId eq '000309' || l.ctgrId eq '000311'}">
                                        <em class="price">
                                    ...
                                    <c:if test="${l.ctgrId ne '000310' && l.ctgrId ne '000303' && l.ctgrId eq '000309' && l.ctgrId ne '000311'}">
                                        <em class="price">
...
<div class="pre_orders mgt70">
    ...
    <div class="pre_pannels frame_box">
        <div class="pannel">
            <ul>
                <c:forEach items="${list6St}" var="l" varStatus="st" end="6">
                    <li class="c${st.count}">
                        <a href="/ht/record/detail/${l.sellPrdtBcode}" title="${l.artiName} - ${l.prdtName}">
...
<div class="best_banner tab_wrap">
    ...
    <c:if test="${ctgrId eq '000300' || ctgrId eq '000301' || ctgrId eq '000302' || ctgrId eq '000303' || ctgrId eq '000308'}">
        <a href="/ht/record/bestList/${ctgrId}?bestRangeType=C1003" class="btn_more">MORE +</a>
    </c:if>
    <c:if test="${ctgrId eq '000304' || ctgrId eq '000309' || ctgrId eq '000310' || ctgrId eq '000311'}">
        <a href="/ht/record/bestList/${ctgrId}?bestRangeType=C1004" class="btn_more">MORE +</a>
    </c:if>
</div>
...
<div class="pre_orders mgt0">
    ...
    <div class="pre_pannels frame_box">
        <div class="pannel">
            <ul>
                <c:forEach items="${list7St}" var="l" varStatus="st" end="6">
                    <li class="c${st.count}">
                        <a href="/ht/record/detail/${l.sellPrdtBcode}" title="${l.artiName} - ${l.prdtName}">
                            <c:if test="${st.first}"><span class="img"><ui:image rcrdCd="${l.rcrdCd}" prdtGbn="${l.sellPrdtGbn}" rcrdImgType="large" width="${l.sellPrdtGbn eq 'R'?'260':'171'}" alt="${l.prdtName}" ratingCode="A"/></span></c:if>
                            <c:if test="${!st.first}"><span class="img"><ui:image rcrdCd="${l.rcrdCd}" prdtGbn="${l.sellPrdtGbn}" width="${l.sellPrdtGbn eq 'R'?'133':'87'}" alt="${l.prdtName}" ratingCode="A"/></span></c:if>
