...
// 		$('.vacant_history').hide();
	...
	// 편집 이벤트
    $('.txt_blue').on('click', function(){  	
    	if($(this).find('em').hasClass('edit')){  		
    		$(this).find('em').attr('class','cancel');
    		$(this).find('em').text('취소');
    		$('#editDiv').show();
    		$('#filterDiv').hide();
    		...
    		$(this).find('em').attr('class','edit');
    		$(this).find('em').text('편집');
    		$('#editDiv').hide();
    		$('#filterDiv').show();
			 ...
			 -->
				...
				<div class="history_inner category_wrap">
					<div class="category_type">
						...
						<div class="txt_blue">
							<a href="javascript:"><em class='edit'>편집</em></a>
						</div>								
...
// 									$('.vacant_history').show();
										...
										<ul class='catalog_history'>
											<c:forEach items="${listRecent0}" var="l" varStatus="st">																
												<li class='${l.rcntGbn} <c:if test="${fn:contains('P,G,S', l.sellPrdtGbn)}">																		
													...
													<c:if test="${l.rcntGbn eq 'ic_03'}">
														<a href="/m/hot/eventDetail?eventId=${l.sellPrdtBcode}" class='btn_erase'>
													</c:if>
													<c:if test="${l.rcntGbn eq 'ic_04'}">
														<a href="/m/hot/eventDetail?eventId=${l.sellPrdtBcode}" class='btn_erase'>
													</c:if>
													<c:if test="${l.rcntGbn eq 'ic_05'}">
														<a href="/m/hot/brandDetail?brandId=${l.sellPrdtBcode}" class='btn_erase'>
													</c:if>																																				
														<c:choose>
													...
													<c:if test="${l.rcntGbn eq 'ic_05'}">
														<a href="javascript:" onclick="addBizWishBrand('${l.sellPrdtBcode}');return false;" class='btn_like<c:if test="${brand.wishUserPrdtCnt ne '0'}"> on</c:if>' id="${l.sellPrdtBcode}"><span>좋아요</span></a><!-- 활성화시 on : class 추가 -->
													</c:if>
					...
					<div class="vacant_history" style="display: none" id="rcnt_vacant">
						...
						<a href="javascript:" onclick="location.href='${defaultHost}/';return false;">메인으로 이동</a>
					</div>					
